"""评测系统 v3 — Agent-as-a-Judge + 原子化Rubric + 证据锚定 + WGR校准核心升级（基于最佳论文方法）：1. 原子化Rubric引擎: 指令特定的检查清单，binary marking，消除主观灰色地带   - 灵感: "Rubric Is All You Need" (Pathak et al. 2025) — QS rubric比generic rubric κ提升314%   - 灵感: RULERS (Hong et al. 2026) — 编译rubric为不可变bundle，evidence-anchored scoring2. Agent-as-a-Judge多步评测: 不是一次性打分，而是Agent自主走完整条评测链   - 灵感: Agent-as-a-Judge (Zhuge et al. 2024) — 显著优于LLM-as-a-Judge   - 步骤: Rubric编译 → 逐项判定(带证据) → 证据验证 → 跨维度校准 → 分数计算3. 证据锚定: 每个判定必须附带对话原文，无证据则分数机械封顶   - 灵感: RULERS Phase II — "If |E_k| < m, score is mechanically capped"4. 确定性校准锚点: 字数/禁用词等代码可验证的约束作为校准锚点   - 灵感: RULERS — 确定性检查和LLM判定分离，确定性检查提供上界5. BCa Bootstrap CI: 比percentile bootstrap更稳健（校正偏差和偏度）6. Gwet's AC1: 比Cohen's Kappa抗悖论（高一致性时Kappa反常低）7. Krippendorff's alpha: 支持多评价者+有序尺度8. pass^k可靠性指标: 测量Agent在多次运行中的一致性（τ-bench风格）"""import sysimport osimport jsonimport timeimport socketimport reimport randomimport mathfrom collections import Countersys.path.insert(0, os.path.dirname(__file__))# ============================================================# LLM 调用（Raw Socket 绕过代理限制）# ============================================================def resolve_ips(host="mmdcadamsminiserverproxy.polaris", port=25340):
